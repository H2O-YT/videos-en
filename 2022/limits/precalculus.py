...
class TeachFunctionScene(Scene):
    ...
    def show_function_mapping(self):
        ...
        self.play(Write(y_f_x[0]))
        self.play(Write(f_x[0][0]))
        self.wait(2)
        self.play(Write(in_out_group[0]))
        self.wait(2)

        for x_tex, y_f_x_tex, f_x_steps, y_tex in zip(in_out_group[0][0], y_f_x[1:], f_x[1:], in_out_group[1][0]):
            self.play(Indicate(x_tex))
            self.wait()
            self.play(
                ReplacementTransform(y_f_x[0], y_f_x_tex),
                ReplacementTransform(f_x[0][0], f_x_steps[0])
            )
            self.wait()
            for i in range(len(f_x_steps)-1):
                self.play(ReplacementTransform(f_x_steps[i], f_x_steps[i+1]))
                self.wait()

            self.play(Circumscribe(f_x_steps[-1]))
            self.play(Indicate(y_f_x_tex))
            self.wait()
            self.play(Write(y_tex))
            self.wait()
            
            arrow = Arrow(x_tex.get_right()+0.2*RIGHT, y_tex.get_left()+0.2*LEFT)
            self.play(Create(arrow))
            self.wait()
            self.play(
                FadeOut(arrow),
                ReplacementTransform(f_x_steps[-1], f_x[0][0]),
                ReplacementTransform(y_f_x_tex, y_f_x[0])
            )
            self.wait()
